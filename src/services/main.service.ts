...
const instance = axios.create({
    ...
    baseURL: 'https://95.154.93.88:32768/'
});
...
export const mainService = {
    ...
    async login({login, password}) {
        const {data} = await instance.get(`api?method=agent.auth&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&login=${ login }&password=${ doubleMd5(password) }`);
        return data;
    },
    async getPaymentsAgent() {
        const {data} = await instance.get<IPaymentsAgent>(`api/?method=agent.payments&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&agentId=${ agentId }`);
        return data;
    },
    async getAllDrivers({offset, count, idAgent}: { offset: number, count: number, idAgent: string }) {
        const {data} = await instance.get<IAllDrivers>(`api/?method=driver.getall&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&agentId=${ idAgent }&offset=${ offset }&count=${ count }`);
        return data;
    },

    async getAllOrders({offset, count, driverId}: { offset: number, count: number, driverId: string }) {
        const {data} = await instance.get<IAllDriverOrders>(`api/?method=driver.orderlist&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&driverId=${ driverId }&offset=${ offset }&count=${ count }`);
        return data;
    },
    ...
    async getDriverInfo({driverId}: { driverId: number }) {
        const {data} = await instance.get<IDriverInfo>(`api/?method=driver.get&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&driverId=${ driverId }`,);
        return data;
    },

    async getCarInfo({driverId}: { driverId: number }) {
        const {data} = await instance.get<ICarInfo>(`api/?method=driver.cars&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&driverId=${ driverId }`);
        return data;
    },
    async getDriverAnalytics({driverId, dateFrom, dateTo}: { driverId: number, dateFrom: string, dateTo: string }) {
        const {data} = await instance.get<IDriverAnalytic>(`api/?method=driver.analytics&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&driverId=${ driverId }&dateFrom=${ dateFrom }&dateTo=${ dateTo }`);
        return data;
    },
    async getDriverTransactions({driverId}: { driverId: string }) {
        const {data} = await instance.get<IDriverTransactions>(`api/?method=driver.transactions&token=a5842677d46463e1f5b9de0db8d0d6aba17b158d8a&driverId=${ driverId }`);
        return data;
    },
    ...
    async createNewCar(requestData: ICarInfo, driverId: string) {
        ...
        formData.append('token', 'a5842677d46463e1f5b9de0db8d0d6aba17b158d8a');
    ...
    async createNewDriver({requestData}: any) {
        ...
        modifiedRequestData.append('token', 'a5842677d46463e1f5b9de0db8d0d6aba17b158d8a');
